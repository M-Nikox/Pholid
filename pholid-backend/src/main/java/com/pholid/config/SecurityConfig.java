...
import com.pholid.audit.AuditLogService;
...
/**
 * Spring Security configuration controlled by the {@code pholid.auth.enabled} property.
 *
 * <ul>
 *   <li>When {@code pholid.auth.enabled=false} (default): all requests are permitted and
 *       OIDC auto-configuration is suppressed so the app starts without Keycloak running.</li>
 *   <li>When {@code pholid.auth.enabled=true}: OAuth2/OIDC login is required for all
 ...
 * pholid.auth.logout-uri. This avoids OIDC discovery (issuer-uri) which fails in
...
public class SecurityConfig {
    ...
    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(name = "pholid.auth.enabled", havingValue = "false", matchIfMissing = true)
    ...
    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(name = "pholid.auth.enabled", havingValue = "true")
    static class SecuredConfig {

        @Bean
        public SecurityFilterChain securedFilterChain(HttpSecurity http,
                AuditLogService auditLogService,
                PholidProperties props) throws Exception {
         ...
         */
        private LogoutSuccessHandler oidcLogoutSuccessHandler(PholidProperties props) {
