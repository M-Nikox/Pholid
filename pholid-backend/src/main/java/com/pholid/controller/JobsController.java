 ...
 *
 * Job history (/history) → Pholid DB, paginated correctly per user.
...
public class JobsController {
     ...
     * the Pholid database. Admins see all jobs; regular users see only their own.
     *
    ...
    /**
     * Maps a Pholid Job entity to the same shape the Flamenco API returns,
     ...
     *   job.metadata['pholid.job_id'] → Pholid job ID  (download link)
    ...
    private Map<String, Object> toFlamencoShape(Job job) {
        ...
        map.put("metadata", Map.of(
                "pholid.job_id", job.getPholidJobId() != null ? job.getPholidJobId() : ""
        ));
        return map;
    }
