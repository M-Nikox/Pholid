 ...
 *  Pholid Render Manager - Active Sessions Module
 */
...
const activeSessions = (() => {
    ...
    function init() {
        ...
        document.addEventListener('pholid:jobSubmitted', handleNewJobSubmitted);

        // When a job completes (notification fired), refresh history
        document.addEventListener('pholid:renderComplete', () => {
            fetchAndDisplayActiveJobs();
            fetchAndDisplayPreviousJobs();
        });
    }
    ...
    function renderPreviousJobs(jobs) {
        ...
        container.innerHTML = jobs.map((job, index) => {
            const displayName = utils.escapeHtml((job.name || 'Untitled').replace(/^Pholid_/, ''));
            ...
            const pholidId = utils.escapeHtml(job.metadata ? (job.metadata['pholid.job_id'] || job.id) : job.id);
            ...
            if (job.status === 'completed') {
                actions.push(`<a href="/api/render/download/${pholidId}" class="text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all" style="background:rgba(34,197,94,0.1);color:#22c55e;border-color:rgba(34,197,94,0.3);">Download</a>`);
            }
            if (showDelete) {
            ...
            return '<div class="history-item history-item-enter px-4 py-3 glass-input border-none flex items-center justify-between group rounded-xl" style="animation-delay:' + staggerDelay + '" data-flamenco-id="' + job.id + '">'
                ...
                +     '<p class="text-xs opacity-50 font-mono mt-0.5">' + pholidId + '</p>'
        ...
        container.querySelectorAll('.history-item[data-flamenco-id]').forEach(row => {
            ...
            row.addEventListener('click', (e) => {
                ...
                document.dispatchEvent(new CustomEvent('pholid:openLogModal', {
    ...
    function createJobCard(job) {
        ...
        const statusInfo = getStatusInfo(job.status);
        const displayName = utils.escapeHtml(job.name.replace(/^Pholid_/, ''));
        const eta = calculateETA(job);
        ...
        card.innerHTML = `
            ...
            <div class="mb-3">
                ...
                <div class="flex justify-between items-center mt-1">
                    ...
                    <span class="job-percent-text text-xs font-semibold" style="color:var(--pholid-orange)">
                        ${progress}%
                    </span>
                </div>
            </div>
            ...
            <!-- ETA -->
            <div class="job-eta-text text-sm mb-3 font-medium" style="color:var(--pholid-orange)" ${eta ? '' : 'style="display:none"'}>
                ⏱️ ${eta || ''}
            </div>
    ...
    function createActionButtons(job) {
        ...
        if (job.status === 'active' || job.steps_completed > 0) {
            const pholidId = job.metadata?.['pholid.job_id'];
            ...
            buttons.push(`
                <button class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-all hover:-translate-y-0.5"
                        onclick="window.location.href='/api/render/download/${pholidId}'"
    ...
    function showCardWarning(card, message) {
        ...
        warn.style.cssText = 'font-size:10px;color:var(--pholid-orange);padding:4px 8px;opacity:0.85;';
