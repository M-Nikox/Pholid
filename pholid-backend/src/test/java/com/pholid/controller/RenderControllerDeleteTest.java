...
import com.pholid.service.JobCleanupService;
...
class RenderControllerDeleteTest {
    ...
    private static final String VALID_PHOLID_ID  = "a1b2c3d4e5f60718";
    ...
    @MockitoBean PholidProperties props;
    ...
    void deleteNotEnabled_returns403() {
        when(props.delete()).thenReturn(new PholidProperties.Delete(false));
    ...
    void invalidFlamencoId_returns400() {
        when(props.delete()).thenReturn(new PholidProperties.Delete(true));
    ...
    private void mockJobWithStatus(String status) {
        Map<String, Object> job = Map.of(
                "status",   status,
                "metadata", Map.of("pholid.job_id", VALID_PHOLID_ID)
        );
        when(flamencoClient.getJob(VALID_FLAMENCO_ID)).thenReturn(job);
    }

    private void mockStoragePath() {
        Path fakePath = Path.of("/shared/jobs", VALID_PHOLID_ID);
        when(storageService.resolveAndValidateJobDir(VALID_PHOLID_ID)).thenReturn(fakePath);
