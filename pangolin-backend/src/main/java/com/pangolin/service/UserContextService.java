 ...
 *
 * <p>When {@code pangolin.auth.enabled=false} (default):
 * <ul>
...
public class UserContextService {
    ...
    public UserContextService(PangolinProperties props, JobRepository jobRepository) {
        this.props         = props;
     ...
     *       {@code pangolin.auth.admin-group} value.</li>
    ...
    public boolean isAdmin() {
        ...
        // Check OIDC groups claim (e.g. Authentik group membership)
        if (auth.getPrincipal() instanceof OidcUser oidcUser) {
    ...
    /**
     * Returns {@code true} if the current user may access the job identified by the
     * given Pangolin job ID (the 16-char hex filesystem directory name).
     */
    public boolean canAccessByPangolinId(String pangolinJobId) {
        if (!props.auth().enabled()) return true;
        if (isAdmin()) return true;
        return jobRepository.existsByPangolinJobIdAndSubmittedBy(
                pangolinJobId, getCurrentUsername());
    }
