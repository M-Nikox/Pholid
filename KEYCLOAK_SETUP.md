# Keycloak Setup — Pangolin v2 (HTTP Dev Mode)
...
## How `PANGOLIN_AUTH_ENABLED` Works
...
**Keep `PANGOLIN_AUTH_ENABLED=false` in `.env` until Step 8.**
...
2. **Realm name:** `pangolin`
   - Must exactly match `KEYCLOAK_REALM` in `.env` (default: `pangolin`)
   - Case-sensitive
3. **Enabled:** On
4. Click **Create**

Verify the top-left dropdown now shows **pangolin** before continuing. Every
...
11. **Valid post logout redirect URIs:** `http://localhost:8080/*`
    - Required for the post-logout redirect back to Pangolin after logout.
...
14. Open the `pangolin` client → **Credentials** tab
...
Pangolin reads a `groups` claim from the OIDC token to determine admin access.
...
1. Open the `pangolin` client → **Client scopes** tab
2. Click `pangolin-dedicated`
3. **Add mapper** → **By configuration** → **Group Membership**
4. Configure exactly as follows:
   - **Name:** `groups`
   - **Token claim name:** `groups`
   - **Full group path:** **Off**
     — if On, the claim value becomes `/pangolin-admins` (with a leading slash),
     which will never match the configured `pangolin-admins` value and admin
...
2. **Name:** `pangolin-admins`
   - Must exactly match `pangolin.auth.admin-group` in `application.properties`
   - Case-sensitive
3. Click **Save**
...
5. **Groups** tab on the user page → **Join group** → select `pangolin-admins` → **Join**

Verify by going to **Groups** → `pangolin-admins` and confirming the user is listed.
...
```
PANGOLIN_AUTH_ENABLED=true
```

### Verify `.env` before continuing

```
KEYCLOAK_REALM=pangolin
...
Watch Pangolin start:

```bash
docker compose -f docker-compose.dev.yml logs -f pangolin-backend
```
...
| # | Check | URL | Expected |
|---|-------|-----|----------|
| 1 | Pangolin loads | `http://localhost:8080` | Redirects to Keycloak login at `http://localhost:8180` |
| 2 | Login works | — | Returns to Pangolin, avatar initials visible top-right |
| 3 | Full name in dropdown | — | Shows first + last name from Keycloak, not just username |
| 4 | Admin panel accessible | `http://localhost:8080/admin` | Page loads (user is in `pangolin-admins`) |
| 5 | Audit log entry | `/admin` | `USER_LOGIN` entry visible for your login |
| 6 | Logout works | — | Redirects to Keycloak, then back to Pangolin login page |
| 7 | Non-admin user blocked | — | Create a second user without `pangolin-admins`, log in, confirm `/admin` returns 403 |
| 8 | Job submission | — | Submit a test job, confirm it appears in history under your username |
| 9 | Grafana SSO (optional) | `http://localhost:3000` | Redirects to Keycloak, logs in, role is Admin for `pangolin-admins` members |

---

## Troubleshooting

### Pangolin redirects to Keycloak but returns an error after login

**Client secret mismatch** — `OIDC_CLIENT_SECRET` in `.env` doesn't match the
`pangolin` client Credentials tab. Regenerate in Keycloak, update `.env`, restart Pangolin.

**Redirect URI mismatch** — in the `pangolin` client Settings tab, confirm
...
1. Confirm the user is in `pangolin-admins`: **Users** → select user → **Groups** tab
2. Confirm the groups mapper exists on `pangolin-dedicated` with **Full group path: Off**
3. If you want to inspect the token directly, open browser dev tools → Network tab
   ...
   `pangolin-admins` (no leading slash)

---

### Logout doesn't redirect back to Pangolin

Confirm **Valid post logout redirect URIs** on the `pangolin` client includes
...
### Pangolin fails to start after enabling auth

```bash
docker compose -f docker-compose.dev.yml logs pangolin-backend
```

- **`OIDC_CLIENT_SECRET` is empty** — Spring rejects an empty secret
- **Realm doesn't exist** — all OIDC URIs return 404. Confirm the `pangolin`
